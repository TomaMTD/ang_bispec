...
def get_binned_B(ell, which_list, lterm, Newton=0, rad=0, isolate=False):
    Nbin = len(ell[:-1])
    ...
    #if rad: 
    #    for ind,w in enumerate(which_list):
    #        if w in ['F2', 'G2', 'dv2']:  which_list[ind] += '_rad'
        ...
        for j in range(i, Nbin):
            for k in range(j, Nbin):
                ...
                try:
                    ...
                    print('all the following file are missing:')
                    for l1 in range(2, ellmax):
                        isfile(l1, lterm, which_list, name)
                    return 0
